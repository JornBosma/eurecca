...
betweenContours = inpolygon(A.DEM.X, A.DEM.Y, pgns.scope(:, 1), pgns.scope(:, 2));

A.DEM.X(~betweenContours) = NaN;
A.DEM.Y(~betweenContours) = NaN;
A.DEM.Z(~betweenContours) = NaN;
B.DEM.X(~betweenContours) = NaN;
B.DEM.Y(~betweenContours) = NaN;
B.DEM.Z(~betweenContours) = NaN;
...
cb.Location = 'northoutside';
set(cb, 'position', [.25 .62 .2 .01])
cb.Label.String = '\leftarrow erosion (m)   deposition \rightarrow';
cb.FontSize = fontsize/1.3;
clim([-2, 2])
